# AGENTS.md — command-type エージェントハーネス

このファイルは、本リポジトリで作業するコーディングエージェント(Claude Code 等)の
行動規範・品質ゲート・完了条件を定義するハーネスである。人間のレビュアもこの規約を前提とする。
末尾の「共通規律」は scaffold-kit 管理領域であり、直接編集しない。

読む順: 本書 → SPEC.md(仕様の正本)→ TEST_SPEC.md → docs/loop-runbook.md。

## 1. プロジェクト概要

Linux コマンド 150 個を「落ちものゲーム × タイピング」で学ぶ練習アプリ。
Next.js (App Router) の静的エクスポート構成で、GitHub 連携の Vercel にデプロイする。

- 仕様の正本は `SPEC.md`。実装とスペックが食い違う場合、スペックが正。
- スペック自体の変更が必要な場合は、**スペック → テスト → 実装**の順に変更する(スペック駆動)。
- 出題データの正本は `data/tracks/{track}.json`(トラック別)。配分の正は SPEC §4(変更はスペック改訂から)。
...
worktree 並列時は `python harness/wtctl.py open/check/close` を用いる(docs/loop-runbook.md)。
PR 前の `check` 合格(差分ゲート + 失敗帰属)は完了条件の一部である。

| 段階 | command-type での実施 |
|---|---|
...
| 5 検証 | `npm run verify`(build 込み)を green にする。品質ゲートコマンドはパイプを通さず素で実行し exit code で判定する |
| 7 完了 | `looplog.py validate` 合格 + `summary` を完了報告に含める |

## 3. 品質ゲート(完了条件)

`npm run verify` が green であること(P1 で package.json とともに整備)。内訳:

| ゲート | 基準 |
|---|---|
| typecheck | `tsc --noEmit` エラー 0 |
| lint | `next lint` エラー 0 |
...
- 落下・照合・スコア・出題のロジックを UI コンポーネント内に直接書かない。必ず core に置きテストを付ける。
- 時間(`requestAnimationFrame`)と乱数(シード)は core の外から**注入**する。core 内で `Date.now()` / `Math.random()` を直接呼ばない(テスト再現性)。
- 状態管理は React 標準(useState 等)のみ。外部状態管理ライブラリを勝手に導入しない。
- 依存パッケージの追加は理由を明記し、原則ゼロランタイム依存追加(devDeps は可)。

## 5. データ規約

- `data/tracks/*.json` の配分(トラック別の件数 / レベル配分 / カテゴリ別件数)は SPEC §4 が正。
  データ変更はスペック改訂 → テスト(T-001〜T-007)→ データの順で行う。
- `desc` は 8〜40 文字の日本語解説。説明として誤りがないこと(不確かなら docs/data-provenance.md の
  参照先で確認)。**書き下ろしオリジナル文のみ** — 既存文書からの転載・翻訳流用は禁止(SPEC §5)。
- テストが落ちるデータを「テスト側の緩和」で通さない。

## 6. 変更禁止領域

- `logs/loops/*.jsonl` — 構造化ループログは append-only(LL-00a)。訂正は correction イベントで行う。
- AGENTS.md 末尾の scaffold ブロックと `.scaffold/manifest.json` — scaffold-kit 管理。
  変更は HARNESS_CHANGELOG 起票 → レジストリ改訂 → `scaffoldctl update` の経路のみ。
- `.wt/gate.json` の上限値 — 変更はレジストリ経由(免除パス・test_command の調整は可)。

## 7. コミット / ブランチ規約(プロジェクト固有分)

共通規律のコミット規約に加えて:

- スペック変更は `spec:` タイプ、データ変更は `data:` タイプを用い、実装と別コミットにする。
- 1 コミット = 1 論理変更。
- 並行タスクのブランチは wtctl が作る `loop/{loop_id}` を用いる(docs/loop-runbook.md)。

## 8. よく使うコマンド

```bash
npm run dev           # 開発サーバ(P1 以降)
npm run verify:fast   # typecheck + lint + test(高速ループ用)
npm run verify        # 上記 + next build(完了条件)

# ループ可観測性(loop-observability)
python harness/looplog.py append --loop loop_XXX --event ... --data ...
python harness/looplog.py validate            # ループ完了条件(LL-11)
python harness/looplog.py summary --loop loop_XXX

# worktree 並列 + 差分ゲート(worktree-kit)— P1 で npm 環境が整ってから使用
python harness/wtctl.py open --loop loop_XXX  # 作成 + ベースライン測定
python harness/wtctl.py check                 # PR 前必須(ゲート + 失敗帰属)
python harness/wtctl.py close --loop loop_XXX # マージ後の片付け(main 側で)

# scaffold(共通規範の版管理)
python ../harness-kit/scaffold-kit/scripts/scaffoldctl.py status --registry ../harness-kit/scaffold-kit/registry
```

<!-- scaffold:block agents_core v1.6.0 -->
